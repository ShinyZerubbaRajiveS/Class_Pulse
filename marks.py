...
def process_marks():
    ...
    file2 = "C:\\Users\\zerub\\OneDrive\\Desktop\\MINI project\\file processing\\marks_data.csv"
    marks_staging = "marks_staging"
    ...
    cursor.execute("""CREATE TEMP TABLE temp_marks (
    RollNo TEXT,
    StudentName TEXT,
    Math TEXT,
    Science TEXT,
    ...
    cursor.execute("TRUNCATE TABLE " + temp_marks + ";")
    conn.commit()
    ...
    sql2 = "COPY temp_marks FROM STDIN WITH (FORMAT csv, HEADER);"
    with open(file2, "r") as file:
        cursor.copy_expert(sql2, file)
    conn.commit()
   
    cursor.execute("TRUNCATE TABLE "+ marks_staging + ";")

    # Process and join the data
    cursor.execute("CALL marks_join_proc();") 
    conn.commit()

    cursor.execute("CALL marks_proc();") 
    conn.commit()
